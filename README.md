...
**Process**:
1. Checks for existing test images in `outputs/` directory (these should be generated by your application's CI pipeline)
...
**Important**: This workflow no longer generates test images. It expects test images to already exist in the `outputs/` directory, typically generated by your application's CI pipeline. If no images are found, the workflow will fail with a clear error message.

**For Testing**: To test the visual diff functionality, use the manual workflow dispatch. This will generate test images AND run the visual comparison automatically.
...
### 2. Test Image Generation and Visual Diff Testing

**Trigger**: Manual workflow dispatch only

**Process**:
1. Generates test images using `generate_test_images.py` for specified scenarios
2. Automatically runs visual diff comparison using the generated images
3. Posts comparison results as PR comment (if any differences found)
4. **Testing workflow only** - not part of normal application CI pipeline

**Usage**: Use this when you want to test the visual diff functionality with known test cases. This will generate the test images AND run the visual comparison in one workflow.
...
**Available scenarios** (for testing only):
...
**Important Change**: The visual diff workflow no longer generates test images automatically. Instead:

1. **Your application's CI pipeline** should generate test images and save them to the `outputs/` directory
2. The visual diff workflow will then compare these existing images against golden masters
3. If no test images are found, the visual diff workflow will fail with a descriptive error

**For Testing the Visual Diff System**:
Use the manual workflow dispatch to both generate test images AND run visual diff testing:

1. **Via GitHub Actions UI** (recommended):
   - Go to Actions → Visual Diff and PR Report → Run workflow
   - Select the desired test scenario from the dropdown
   - Click "Run workflow"
   - This will generate test images AND run the visual comparison automatically
...
Your application's CI should populate the `outputs/` directory with screenshots, renders, or other visual outputs that need to be tested. For example:
