...
**For Testing**: To generate test images for testing purposes, use the manual workflow dispatch with the "Generate Test Images" option.
...
### 2. Generate Test Images (Testing Only)

**Trigger**: Manual workflow dispatch only

**Process**:
1. Generates test images using `generate_test_images.py` for specified scenarios
2. Uploads generated images as artifacts for download
3. **Not part of the main CI pipeline** - only for testing the visual diff system

**Usage**: Use this when you want to test the visual diff functionality with known test cases.
...
Use the manual workflow dispatch "Generate Test Images" job to create test images for testing purposes:

1. **Via GitHub Actions UI** (recommended):
   - Go to Actions → Visual Diff and PR Report → Run workflow
   - Select the desired test scenario from the dropdown
   - Click "Run workflow"
   - This will generate test images as artifacts (not committed to the repository)
